...
/** Ponto de entrada do jogo: exibe o menu principal e acumula o histórico de batalhas. */
public class Main {
  ...
  /** Inicia o jogo, monta o treinador do jogador e mantém o laço do menu principal. */
  public static void main(String[] args) {
    ...
    Mochila mochilaJogador =
        new Mochila(new ArrayList<>(List.of(Itens.POTION, Itens.SUPERPOTION, Itens.ANTIDOTE)));
    ...
    do {
      ...
      switch (opcao) {
        case 1:
          iniciarBatalha(jogador, random, scanner);
          break;
        case 2:
          verMochila(jogador);
          break;
  ...
  private static void iniciarBatalha(Treinador jogador, Random random, Scanner scanner) {
    Treinador cpu =
        new Treinador("CPU", sortearEscolhaCpu(random), new Mochila(new ArrayList<>()));
    ...
    if (vencedor == jogador) {
      ...
      System.out.println(
          "Recompensa obtida: " + recompensa.getNome() + " adicionado à mochila!");
    } else {
      derrotas++;
  ...
  private static SalDex escolherPokesal(Scanner scanner) {
    ...
    for (int i = 0; i < opcoes.length; i++) {
      System.out.println(
          (i + 1) + " - " + opcoes[i].getNome() + " (" + opcoes[i].getTipoElemental() + ")");
    }
